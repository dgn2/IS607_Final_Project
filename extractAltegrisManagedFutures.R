...
extractAltegrisData<-function (outputDirectory){
  ...
  for (programIndex in seq_along(programURLs[1:20])){
    # extract program ID
...
# output file path
outputDirectory<-'C:/Users/dgn2/Documents/R/IS607/Final_Project/altegris/'
...
loadCtaProgramInfoTable <- function (dbHandle,outputDirectory,
  ...
  query<-paste0("LOAD DATA LOCAL INFILE '",outputDirectory,
                ...
                "(column_name,column_value,column_type,cta_name,program_name,program_id);")
  #
  dbGetQuery(dbHandle,query)
}
